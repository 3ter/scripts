...
my $is_first_sync = 0;
GetOptions( 'first-sync' => \$is_first_sync )
...
sub check_net_connectivity {
    my $p = Net::Ping->new();
    $p->port_number(443);
    if ( !$p->ping('drive.google.com') ) {
        say "No connection to 'drive.google.com'";
        return 0;
    }
    return 1;
}
...
sub run_sync {
    ...
    my $timeout = 10 * 60;
    while ( !check_net_connectivity() ) {
        die "Still no connection, giving up" if ( $timeout <= 0 );
        sleep(60);
        $timeout - 60;
    }
    ...
    if ($is_first_sync) {
        say
"You've specified '--first-sync'. This will overwrite files in $remote_dir "
        ...
        if ( $answer eq 'y' ) {
            say
qx( python3 $rclonesync_exe --first-sync --verbose $local_dir $remote_dir 2>&1 );
        }
    }

    my $rclonesync_rc = (
        system("python3 $rclonesync_exe --verbose $local_dir $remote_dir 2>&1")
          >> 8 );
    system(
"/usr/bin/notify-send --urgency critical 'Error in rclone-sync' 'Ended with return code $rclonesync_rc'"
    ) if ($rclonesync_rc);
